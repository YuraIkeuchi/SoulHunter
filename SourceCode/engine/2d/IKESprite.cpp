...
/// 静的メンバ変数の実体
/// </summary>
ID3D12Device* IKESprite::device = nullptr;
UINT IKESprite::descriptorHandleIncrementSize;
ID3D12GraphicsCommandList* IKESprite::cmdList = nullptr;
ComPtr<ID3D12RootSignature> IKESprite::rootSignature;
ComPtr<ID3D12PipelineState> IKESprite::pipelineState;
XMMATRIX IKESprite::matProjection;
...
bool IKESprite::LoadTexture(UINT texnumber, const wchar_t* filename)
{
	// nullptrチェック
	assert(device);
...
void IKESprite::PreDraw()
{
	//// PreDrawとPostDrawがペアで呼ばれていなければエラー
	//assert(IKESprite::cmdList == nullptr);
...
void IKESprite::SetRotation(float rotation)
{
...
void IKESprite::SetPosition(const XMFLOAT2& position)
{
	this->position = position;
...
void IKESprite::SetSize(const XMFLOAT2& size)
{
	this->size = size;
...
void IKESprite::SetIsFlipX(bool isFlipX)
{
	this->isFlipX = isFlipX;
...
void IKESprite::SetIsFlipY(bool isFlipY)
{
	this->isFlipY = isFlipY;
...
void IKESprite::SetTextureRect(const XMFLOAT2& texBase, const XMFLOAT2& texSize)
{
...
void IKESprite::TransferVertices()
{
	HRESULT result = S_FALSE;
